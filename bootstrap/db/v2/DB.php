...
namespace bemibbs\db\v2;

use mysqli;

/**
 * @author Lingqi <3615331065@qq.com>
 * @time 2022-05-18 21:32
 */
class DB
{
    //MySQL主机地址
    ...
    protected array $_validate        =   array();  // 自动验证定义
    protected array $_auto            =   array();  // 自动完成定义
    protected array $_map             =   array();  // 字段映射定义
    protected array $_scope           =   array();  // 命名范围定义
    // 链操作方法列表
     ...
     * @param int $port
     */
    public function __construct(string $host, string $user, $password, string $database, int $port=3306){
        $this->_initialize();
     ...
     * @return array 数据表中指定字段信息
     * */
    protected function query_handle($obj): array
    {
        $res = array();
     ...
     * @return object $this
     * */
    public function limit($limit): DB
    {
     ...
     * @return object $this
     * */
    public function order($order,$type='desc'): DB
    {
    ...
    public function add(){
        ...
        return mysqli_query($this->_dbObj,$sql);
    }
    /*
     * 数据更新函数（一维数组）
     * @access public
